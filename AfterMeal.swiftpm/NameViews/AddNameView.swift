...
struct AddNameView: View {
    ...
    func useEffect() -> Void {
        foodList.forEach { food in
            foodListCheck.append(FoodCheck(Name: food.Name, IsCheck: true, Category: food.Category))
            let category = food.Category
            if(!categoryList.contains(category)) {
                categoryList.append(category)
                categoryListCheck.append(CategoryCheck(Name: category, count: 1, len: 1, IsCheck: true))
            } else {
        ...
        self.eatCount = foodListCheck.count
    }
                ...
                VStack(spacing: 10) { //form
                    ...
                    VStack(spacing: 10) { // Save
                        Button(action: {
                            ...
                            nameList.append(NameClass(Name: Name, Group: Group, Foods: foods, Categories: categories, EatAll: eatAll, Photo: Photo))
                            dismiss()
                        }, label: {
                            Text("Save")
                                .font(.system(size: 20))
